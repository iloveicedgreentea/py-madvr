...
class Madvr:
    ...
    async def task_handle_queue(self) -> None:
        """Handle command queue with improved error handling."""
        while True:
    ...
    async def task_read_notifications(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                if not msg:
                    self.logger.warning(
                        "Empty message received. Could be a device issue"
                    )
                    await self._handle_power_off()
                    continue
    ...
    async def is_device_connectable(self) -> bool:
        """Check if the device is connectable with timeout."""
        try:
